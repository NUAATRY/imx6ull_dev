				...
				size_t count, loff_t *off)
{
	...
	DECLARE_WAITQUEUE(wait,current);  //创建一个等待任务
	if(atomic_read(&dev->keyrelease) == 0){
		add_wait_queue(&dev->r_wait,&wait);  /* 加入到等待队列头*/
		__set_current_state(TASK_INTERRUPTIBLE); /*设置为只有信号可以打断*/
		schedule();  /*切换任务，进入休眠态*/
		if(signal_pending(current)){ /*由信号唤醒不是中断*/
			ret = -ERESTARTSYS;
			goto wait_error;
		}
		__set_current_state(TASK_RUNNING);  /*运行此任务*/
		remove_wait_queue(&dev->r_wait,&wait);  /*将任务删除*/
	}
